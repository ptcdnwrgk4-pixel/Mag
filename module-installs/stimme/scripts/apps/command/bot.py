...
async def _transcribe_voice(bot: Bot, message: Message) -> str | None:
    """Download a Telegram voice note and transcribe via OpenRouter Whisper.

    Uses OpenRouter's /audio/transcriptions endpoint with whisper-large-v3-turbo.
    Request format: JSON body with base64-encoded audio (NOT multipart/form-data).

    Returns the transcribed text, or None if transcription fails
    (missing API key, API error, etc.).
    """
    if not _config or not _config.openrouter_api_key:
        msg_log.warning("Voice note received but OPENROUTER_API_KEY not configured")
        return None

    try:
        import httpx
    except ImportError:
        msg_log.error("httpx package not installed: cannot transcribe voice notes")
        return None
    ...
    try:
        ...
        # Base64-encode for OpenRouter (raw bytes, not data URI)
        audio_b64 = base64.b64encode(audio_bytes).decode("ascii")

        # Call OpenRouter transcription endpoint
        ...
        # OpenRouter returns the transcription in the "text" field (OpenAI-compatible).
        # If the field name changes, this is the single line to adjust.
        text = (data.get("text") or "").strip()
        if not text:
            msg_log.error("Empty transcription response: %s", data)
            return None
        ...
        msg_log.error(
            "OpenRouter transcription failed: HTTP %d — %s",
...
async def handle_reboot(message: Message) -> None:
    ...
    await message.reply("Neustart läuft...")
...
async def handle_message(message: Message) -> None:
    ...
    if message.voice:
        ...
        await message.reply("Sprachnachricht wird verarbeitet...")
        text = await _transcribe_voice(_orchestrator.bot, message)
        if text:
            _enqueue_and_debounce(_BufferedItem(message=message, text=text))
        else:
            await message.reply(
                "Sprachnachricht konnte nicht verarbeitet werden. Check OPENROUTER_API_KEY in .env."
            )
        return
