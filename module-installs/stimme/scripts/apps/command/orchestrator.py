...
class Orchestrator:
    ...
    async def handle_general_message(
        ...
        if text.startswith("/reset"):
            ...
            await message.reply("Sitzung zurückgesetzt. Die nächste Nachricht startet frisch.")
            return
    ...
    async def handle_agent_topic_message(
        ...
        if text.startswith("/reset"):
            ...
            await message.reply("Sitzung zurückgesetzt. Die nächste Nachricht lädt den Kontext neu.")
            return
        if text.startswith("/compact"):
        ...
        if not session:
            await message.reply("Keine Sitzung für dieses Topic. Vermutlich zurückgesetzt.")
            return
        ...
        if input_tokens > self.config.context_warning_threshold:
            ...
            await self.bot.send_message(
                ...
                text=(
                    ...
                    "Mit /compact komprimieren oder mit /new einen frischen Agenten starten."
                ),
                message_thread_id=thread_id,
            )
    ...
    async def _rename_topic(self, message: Message) -> None:
        ...
        if not topic_id:
            await message.reply("Nutze /name innerhalb eines Agent-Topics, um es umzubenennen.")
            return

        topic_key = str(topic_id)
        session = self.sessions.get(topic_key)
        if not session:
            await message.reply("Keine Agent-Sitzung für dieses Topic.")
            return
        ...
        if result.is_error:
            await message.reply("Agent-Fehler. Probier /reset und danach nochmal /name.")
            return
        ...
        if not new_name:
            await message.reply("Agent hat keinen Namensvorschlag zurückgegeben.")
            return
    ...
    async def _compact_session(self, topic_key: str, message: Message) -> None:
        ...
        session = self.sessions.get(topic_key)
        if not session:
            await message.reply("Keine aktive Sitzung zum Komprimieren.")
            return

        await message.reply("Kontext wird komprimiert...")
    ...
    async def _cmd_help(self, message: Message) -> None:
        ...
        await message.reply(
            "<b>Dein Mitarbeiter</b>\n\n"
            "Schreib mir hier im General-Topic. Ich bin dein Mitarbeiter mit vollem "
            "Workspace-Zugriff.\n\n"
            "<b>Befehle:</b>\n"
            "/new [sonnet|opus] \u2014 Frischen Agent in neuem Topic starten\n"
            "/name \u2014 Aktuelles Topic anhand der Unterhaltung umbenennen\n"
            "/compact \u2014 Kontext komprimieren, wenn er voll wird\n"
            "/reset \u2014 Sitzung zur\u00fccksetzen und neu anfangen\n"
            "/help \u2014 Diese Nachricht\n\n"
            "<b>Tipps:</b>\n"
            "\u2022 Mit /name nach einer Unterhaltung das Topic einen sprechenden Namen geben\n"
            "\u2022 Mit /compact arbeiten, wenn der Agent fr\u00fcheren Kontext zu vergessen anf\u00e4ngt\n"
            "\u2022 Sprachnachrichten, Fotos und Screenshots schicken \u2014 der Agent sieht alles\n\n"
            "Antworte in einem beliebigen Agent-Topic, um mit vollem Kontext weiterzumachen.",
            parse_mode="HTML",
        )
