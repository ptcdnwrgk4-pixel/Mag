"""
Intelligenz: Datenbank und Hilfsfunktionen.

Legt die SQLite-Datenbank an und verwaltet sie. Hält Meeting-Mitschriften,
Slack-Nachrichten, das Team-Register und das Sammel-Log.
"""
...
# Datenbank liegt im data/-Ordner relativ zum Workspace-Root
WORKSPACE_ROOT = Path(__file__).resolve().parent.parent
DB_PATH = WORKSPACE_ROOT / "data" / "intel.db"


SCHEMA_SQL = """
-- Meeting-Mitschriften aus jedem Recorder (Fireflies, Fathom, eigener)
...
-- Slack-Nachrichten (eine Zeile pro Nachricht)
...
-- Team-Register (fuer die Meeting-Einordnung)
...
-- Sammel-Log (haelt jeden Lauf fest)
...
def get_connection() -> sqlite3.Connection:
    """Datenbank-Verbindung holen. Legt Datei und Tabellen bei Bedarf an."""
...
def init_db() -> sqlite3.Connection:
    """Datenbank mit allen Tabellen initialisieren."""
    conn = get_connection()
...
def write_meetings(conn: sqlite3.Connection, meetings: list[dict]) -> int:
    """Meeting-Datensaetze schreiben. Gibt die Anzahl zurueck."""
    records = 0
    now = _now_iso()
    for m in meetings:
...
def write_slack(conn: sqlite3.Connection, messages: list[dict]) -> int:
    """Slack-Nachrichten schreiben. Gibt die Anzahl zurueck."""
    records = 0
    now = _now_iso()
    for msg in messages:
...
def write_staff(conn: sqlite3.Connection, staff_list: list[dict]) -> int:
    """Team-Mitglieder ins Register schreiben. Gibt die Anzahl zurueck."""
    now = _now_iso()
    count = 0
...
def get_daily_slack_transcript(conn: sqlite3.Connection, date: str,
    ...
    """Slack-Tagestranskript als lesbaren Text holen.

    Gibt alle Nachrichten eines Tages zurueck, im Format:
    [#kanal] absender: nachricht

    Praktisch fuer Zusammenfassungen oder Volltext-Suche.
    """
    query = """
    ...
    if not rows:
        return f"Keine Slack-Nachrichten fuer {date} gefunden."

    lines = []
    for row in rows:
        channel = row["channel_name"] or "unbekannt"
        user = row["user_name"] or "unbekannt"
        text = row["text"] or ""
...
def search_meetings(conn: sqlite3.Connection, query: str,
    ...
    """Meeting-Mitschriften und Titel nach Stichwort durchsuchen.

    Gibt passende Meetings mit Titel, Datum und Snippet zurueck.
    """
    rows = conn.execute(
...
def search_slack(conn: sqlite3.Connection, query: str,
                  days: int = 30) -> list[dict]:
    """Slack-Nachrichten nach Stichwort durchsuchen.

    Gibt passende Nachrichten mit Kanal, Absender und Text zurueck.
    """
    rows = conn.execute(
...
def get_meeting_stats(conn: sqlite3.Connection) -> dict:
    """Schnelle Zusammenfassung dessen, was in der Datenbank steht."""
