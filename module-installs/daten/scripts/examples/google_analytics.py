"""
Daten: Google Analytics 4 Sammler (Beispiel)

Holt Webseiten-Traffic, Quellen und Engagement aus GA4.
Greift die Daten von gestern ab, als Tagesstand.

Diese Datei nach scripts/collect_google_analytics.py kopieren, um sie zu aktivieren.

Voraussetzungen:
    GOOGLE_SERVICE_ACCOUNT_JSON : Pfad zur Service-Account-JSON
    GA4_PROPERTY_ID             : deine GA4-Property-ID

Erzeugte Tabellen: ga4_daily, ga4_sources
Zusätzliches pip: google-analytics-data google-auth
"""
...
try:
    ...
    raise ImportError(
        "Pakete fehlen, installier sie mit: pip install google-analytics-data google-auth"
    )


def _get_client():
    """Authentifizierten GA4-Client anlegen."""
...
def collect():
    """GA4-Traffic-Daten von gestern holen."""
    property_id = os.getenv("GA4_PROPERTY_ID", "").strip()
    if not property_id:
        return {
            "source": "google_analytics", "status": "skipped",
            "reason": "GA4_PROPERTY_ID fehlt, finden unter "
...
def write(conn, result, date):
    """GA4-Daten in die Datenbank schreiben. Gibt Anzahl der Datensätze zurück."""
    conn.execute("""
...
if __name__ == "__main__":
    ...
    if result["status"] == "success":
        ...
        print(f"GA4-Daten vom {result['data']['date']}:")
        print(f"  Sitzungen: {ov.get('sessions', 'N/A')}")
        print(f"  Nutzer: {ov.get('totalUsers', 'N/A')}")
        print(f"  Seitenaufrufe: {ov.get('screenPageViews', 'N/A')}")
    else:
